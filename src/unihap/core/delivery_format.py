"""
==============================================================================
FILE: src/unihap/core/delivery_format.py
MODULE: Delivery Format Exporter (252 Columns)
PURPOSE:
    Transforms internal EnrichedProductRecord objects into the exact 252-column
    ground-truth delivery CSV format expected by the Unihack benchmark and enterprise
    downstream systems.

HEADERS MAPPED (252 columns):
    - Source & Verification: MFR URL, Ref URL 1..5
    - Identifiers: PART_NUMBER, SKU, Mfg_Part_Num, Part_Desc, Manufacturer, Brand
    - Canonical Master Data: MANUFACTURER_NAME, BRAND_NAME, Classpath (Dept, Class, Fine)
    - 5 Description Formats: MOBILE_DESC, INVOICE_DESC, SHORT_DESC, LONG_DESC1, RETAIL_DESC, MARKETING_DESCRIPTION
    - Feature Bullet Points: ITEM_FEATURES_1 through ITEM_FEATURES_20
    - Attribute Triplets: ATTRIBUTE_LABEL 1..50, ATTRIBUTE_VALUE 1..50, ATTRIBUTE_UOM 1..50
    - Commercial & Physical: UPC, EAN, GTIN, UNSPSC, Dimensions (L/W/H/Weight/Volume)
    - Digital Assets: Product Image, Alternate Images 1..4, Spec Sheets, SDS, Line Drawings

CLASSES:
    - DeliveryFormatExporter: Static mapping and batch CSV writer.
==============================================================================
"""
...
class DeliveryFormatExporter:
    ...
    def record_to_delivery_row(record: EnrichedProductRecord) -> Dict[str, Any]:
        ...
        # URLs
        row["MFR URL"] = record.spec_source_url or (
            f"https://{record.manufacturer_domain}" if record.manufacturer_domain else ""
        )
